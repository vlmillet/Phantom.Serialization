...
struct ValuePrimitiveEqualitySignDependant
{
    ...
    static bool isSame(T0 v0, T1 v1) { return (type)v0 == (type)v1; }
};
...
{
    static bool isSame(double v0, float v1, double epsilon) { return std::abs(v0 - double(v1)) <= epsilon; }
};

template<>
struct ValuePrimitiveEquality<float, double>
{
    static bool isSame(float v0, double v1, double epsilon) { return std::abs(v1 - double(v0)) <= epsilon; }
};
...
{
    static bool isSame(T0 v0, T0 v1) { return v0 == v1; }
};

template<>
struct ValuePrimitiveEquality<double, double>
{
    static bool isSame(double v0, double v1, double epsilon) { return std::abs(v0 - v1) <= epsilon; }
};

template<>
struct ValuePrimitiveEquality<long double, long double>
{
    static bool isSame(long double v0, long double v1, long double epsilon) { return std::abs(v0 - v1) <= epsilon; }
};

template<>
struct ValuePrimitiveEquality<float, float>
{
    static bool isSame(float v0, float v1, float epsilon) { return std::abs(v0 - v1) <= epsilon; }
};

class PHANTOM_EXPORT_PHANTOM_SERIALIZATION Value
{
public:
    struct Separator
    {
    ...
    Value(const Value& copied) : m_type(copied.m_type)
    {
        ...
        if (m_type == e_struct)
        {
            m_struct = new_<Struct>(*copied.m_struct);
        }
        else if (m_type == e_array)
        {
            m_array = new_<Array>(*copied.m_array);
        }
    ...
    explicit Value(hex64 hex) : m_type(e_hex64) { m_hex64 = hex; }
    ...
    Value& operator=(String c) { return operator=(StringView(c)); }
...
#undef _value_ctor_operator_as
    ...
    bool as(StringView& val) const
    {
        PHANTOM_ASSERT(!isNull());
        if (m_type == e_cstr)
        {
            val = StringView(m_cstr, m_cstr_size);
            return true;
        }
        return false;
    }
...
#define _value_equals_member_function_epsilon(type)                                                                    \
    ...
    inline bool isArray() const { return m_type == e_array; }
    inline bool isStruct() const { return m_type == e_struct; }

    inline bool isDefined() const { return m_type != e_undefined; }
    ...
    PHANTOM_FORCEINLINE bool isNull() const { return this == &null; }
    PHANTOM_FORCEINLINE bool isValid() const { return this != &null; }
...
private:
    ...
    inline void _release()
    {
        if (m_type == e_struct)
        {
            delete_<Struct>(m_struct);
        }
        else if (m_type == e_array)
        {
            delete_<Array>(m_array);
        }
        else if (m_type == e_cstr)
        {
            PHANTOM_FREE(m_cstr);
